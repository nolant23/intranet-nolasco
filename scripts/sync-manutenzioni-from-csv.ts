/**
 * Sincronizza le manutenzioni dal CSV: se esiste già un record con lo stesso glideId
 * lo aggiorna (inclusi semestrali e firme), altrimenti lo crea.
 * Se la colonna "rapportino" del CSV contiene un URL, il PDF viene scaricato e caricato su Supabase;
 * altrimenti viene generato con generateRapportinoPDF e caricato.
 *
 * Per una re-importazione da zero: prima eseguire delete-all-manutenzioni.ts, poi questo script.
 *
 * Eseguire dalla root: npx tsx scripts/sync-manutenzioni-from-csv.ts "/path/to/MAN_ELENCO MANUTENZIONI.csv"
 */
...
/** Split del file CSV in righe logiche: i newline dentro campi tra virgolette non spezzano la riga. */
...
async function main() {
  ...
  const toProcess: CsvRow[] = [];
  const skippedNoImpiantoOrTecnico: CsvRow[] = [];
  for (const row of csvRows) {
    const impiantoInDb = impiantiByNum.has(row.impianto);
    const tecnicoInDb = tecniciByName.has(row.tecnico.toLowerCase());
    if (impiantoInDb && tecnicoInDb) toProcess.push(row);
    else skippedNoImpiantoOrTecnico.push(row);
  }

  const totalRigheDati = lines.length - 1;
  const righeConDataValida = csvRows.length;
  const esclusePerData = totalRigheDati - righeConDataValida;

  console.log("\n--- Sync manutenzioni da CSV ---");
  console.log("Righe dati nel CSV (esclusa intestazione):", totalRigheDati);
  if (esclusePerData > 0) console.log("Righe escluse (data non valida o impianto/tecnico vuoti):", esclusePerData);
  if (skippedNoImpiantoOrTecnico.length > 0) {
    console.log("Righe escluse (impianto o tecnico non presenti in DB):", skippedNoImpiantoOrTecnico.length);
    for (const r of skippedNoImpiantoOrTecnico) {
      const motivoImpianto = impiantiByNum.has(r.impianto) ? "" : "impianto non in DB";
      const motivoTecnico = tecniciByName.has(r.tecnico.toLowerCase()) ? "" : "tecnico non in DB";
      console.log(`  Riga ${r.rowNum} | Impianto: ${r.impianto} | Tecnico: ${r.tecnico} → ${[motivoImpianto, motivoTecnico].filter(Boolean).join(", ") || "?"}`);
    }
  }
  console.log("Righe da processare (impianto+tecnico in DB):", toProcess.length);

  let created = 0;
  let updated = 0;
  let errors = 0;

  const buildPayload = (row: CsvRow, impiantoId: string, tecnicoId: string, dataManutenzione: Date) => ({
    dataManutenzione,
  ...
  /** Se il CSV ha un URL rapportino, lo scarica e carica su Supabase; altrimenti genera il PDF e lo carica. */
  ...
  for (let i = 0; i < toProcess.length; i++) {
    ...
    const startOfDay = new Date(dataManutenzione);
    startOfDay.setHours(0, 0, 0, 0);
    const endOfDay = new Date(dataManutenzione);
    endOfDay.setHours(23, 59, 59, 999);

    try {
      const existing =
        row.glideId
          ? await prisma.manutenzione.findUnique({ where: { glideId: row.glideId } })
          : await prisma.manutenzione.findFirst({
              where: {
                impiantoId,
                tecnicoId: tecnico.id,
                dataManutenzione: { gte: startOfDay, lte: endOfDay },
              },
            });
      ...
      if (existing) {
        await prisma.manutenzione.update({
          ...
          data: {
            ...buildPayload(row, impiantoId, tecnico.id, dataManutenzione),
            ...(row.glideId ? { glideId: row.glideId } : {}),
            firmaCliente,
            firmaTecnico,
          },
        });
        await setRapportinoPdf(existing.id, row.rapportinoUrl);
        updated++;
      } else {
  ...
  console.log("\n--- Fine: create", created, "aggiornate", updated, "errori", errors);
}
