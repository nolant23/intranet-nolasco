...
export default async function TecnicoPresenzeDashboardPage() {
  const user = await getCurrentUser();
  if (!user) redirect("/login");
  const permissions = await getPermissionsForRole(user.role);
  const canPresenze = permissions?.Presenze?.READ;
  const canPresenzeCantiere = permissions?.Tecnici?.READ || permissions?.PresenzeCantiere?.READ;
  if (!canPresenze && !canPresenzeCantiere) redirect("/tecnici");
